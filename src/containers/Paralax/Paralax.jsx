...
class Paralax extends Component {
  ...
  onScroll() {
    const { layers } = this;
    const titleStyle = this.titleRef.current.current.current.style;
    console.log('keks')
    if (window.innerWidth > 980) {
      console.log(window.scrollY, this.containerRef.current.offsetHeight)
      if (window.scrollY < this.containerRef.current.offsetHeight - this.settings.title.offset.top) {
        layers.forEach(layer => {
          const element = layer.ref.current.current.current;
          console.log(element)
        ...
        <container ref={this.containerRef}>
          {
            this.layers.map(layer => {
              return <Layer ref={layer.ref} key={layer.id} image={layer.image} />
            })
          }
        </container>
        <Title ref={this.titleRef} offsetTop={this.settings.title.offset.top} />
      </content>
